...
HeatCanvas.prototype._render = function(f_value_color){
    ...
    if (this.width != this.valueWidth || this.height != this.valueHeight)
        return;

    var valLen = this.value.length;
    if (valLen != this.valueWidth * this.valueHeight) {
        console.log('inconsistent data w*h != len', this.valueWidth, this.valueHeight, '!=', valLen);
        return;
    }
    ...
    for (var id = 0; id < valLen; id++) {
        ...
        if (minValue === null || minValue > this.value[id])
            minValue = this.value[id];
    }

    console.log(maxValue, 'scaled:', Math.log(maxValue + 1), minValue, 'scaled:', Math.log(minValue + 1), 'useLog:', useLog);
    if (useLog) {
        maxValue = Math.log(maxValue + 1);
        minValue = Math.log(minValue + 1);
    }
    minValue = 0;
...
HeatCanvas.prototype._ensureDefaultPalette = function() {
    ...
    for (var j = 0; j <= this._paletteSize; j++)
        res[j] = HeatCanvas.hsla2rgba.apply(null, HeatCanvas.defaultValue2Color(j / this._paletteSize));
...
HeatCanvas.hsla2rgba = function(h, s, l, a){
    ...
    if (Uint8ClampedArray.of)
        return Uint8ClampedArray.of(r*255, g*255, b*255, a*255);
